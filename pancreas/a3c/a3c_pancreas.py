...
parser = argparse.ArgumentParser(description='A3C runner for simglucose')
...
class ActorCriticModel(keras.Model):
    ...
    def call(self, inputs):
        '''
        This performs our forward pass operation for our
        model-embedded neural network scheme

        Parameters
        ----------
        inputs - Our input feature vector
        '''
        # Forward pass
...
class MasterAgent():
    ...
    def __init__(self):
        self.game_name = 'simglucose-adolescent2-v0'
        self.save_dir = './trained'
...
class Worker(threading.Thread):
        ...
        while Worker.global_episode < args.max_eps:
            ...
            while not done:
                ...
                if time_count == args.update_freq or done:
                    # Calculate gradient wrt to local model. We do so by
                    # tracking the variables involved in computing the loss
                    # by using tf.GradientTape
                    ...
                    # Clear memory now that we're done
                    mem.clear()

                    # When done, print info
                    if done:
                        Worker.global_moving_average_reward = \
                            record(Worker.global_episode,
                                   ep_reward,
                                   self.worker_idx,
                                   Worker.global_moving_average_reward,
                                   self.result_queue,
    ...
    def compute_loss(self,
        ...
        logits, values = self.local_model(
                tf.convert_to_tensor(np.vstack(memory.states),
                                     dtype=tf.float32))
        # Get our advantages
        # Represented as R - V(s)
        advantage = tf.convert_to_tensor(np.array(discounted_rewards)[:, None],
                                         dtype=tf.float32) - values

        # Value loss
        # Make it a value loss by squaring it
        ...
        total_loss = tf.reuce_mean((0.05 * value_loss + policy_loss))
    ...
    def play(self):
        ...
        state = env.reset()
        model = self.global_model
        model_path = os.path.join(self.save_dir, 'model_{}.h5'.format(
            self.game_name))
        print('Loading model from: {}'.format(model_path))
        model.load_weights(model_path)
        done = False
